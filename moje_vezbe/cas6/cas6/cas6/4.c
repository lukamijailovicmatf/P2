...
/* Funkija poredjenja koja ce se koristiti za pronalazak studenta sa zadatim prezimenom, u nizu studenata sortiranih po prezimenu */
...
/* Funkcija koja binarnom pretragom pronalazi prvog studenta sa zadatim prezimenom */
int binarna_rekurzivno(STUDENT* niz, int l, int d, char* pr){
  ...
  if(rez == 0){
    if(s == 0)
      return s;
    else if(strcmp(niz[s-1].prezime, pr) == 0)
