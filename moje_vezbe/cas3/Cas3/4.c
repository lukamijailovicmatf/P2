...
int main(int argc, char *argv[]){
     ...
     * Kada koristimo ovu funkciju ne moramo da vodimo racuna o alokaciji prostora za cuvanje ucitane linije - ova funkcija ce to uraditi za nas. 
     * Ako je pozovemo sa:
     ...
     * Ako je linija neki pokazivac koji je prethodno vec alociran, onda ce funkcija getline
       ucitavati u tu memoriju i po potrebi odraditi realokaciju i azurirati vrednost promenljive
       duzina_linije.
    ...
    while(getline(&linija, &duzina_linije, ulaz) != EOF){
       ...
       * Za razliku od fscanf-a i scanf-a koji imaju mehanizam da se zapamti gde se stalo sa citanjem
         ...
         sscanf-a i iz tog razloga, sami moramo da obezbedimo da nam ovo citanje radi ispravno.
         
       * Ako pozovemo: 
           sscanf("cao svima", "%s%d", rec, &broj);
       ...
       * Tako da ako sledeci put pozovemo sa:
           sscanf("cao svima" + 3, "%s%d", reci, &broj);
         zapravo vrsimo poziv:
           sscanf(" svima", "%s%d", reci, &broj);
         u ovom slucaju, u promenljivu rec ce biti upisano "svima", a u broj 6 (racuna se i ona belina koja je preskocena)
      ...
      while(sscanf(linija + procitano, "%s%d", rec, &k) != EOF){
      ...
      /* Ako je brojac jednak trazenom broju pojavljivanja, ispisujemo tekucu liniju na standardni izlaz */
